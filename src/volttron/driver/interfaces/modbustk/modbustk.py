...
class ModbusTKRegister(BaseRegister):
    ...
    def __init__(self, point_name, default_value, field, description=''):
        ...
        super(ModbusTKRegister, self).__init__(datatype,
                                               not field.writable,
                                               point_name,
                                               field.units,
    ...
    def get_python_type(self, datatype):
        ...
        struct_types = [
            type(x) for x in parse_struct.unpack(('\x00' * parse_struct.size).encode('utf-8'))
        ]
...
class ModbusTK(BasicRevert, BaseInterface):
    """
    ...
    def configure(self, config_dict, registry_config_lst):
        ...
        write_single_values = not helpers.str2bool(
            str(config_dict.get('write_multiple_registers', "True")))
        ...
        register_map = dict((reg['register name'], reg) for reg in [
            dict((k.lower(), v) for k, v in i.items())
            for i in config_dict.get('register_map', registry_config_lst)
        ])
        ...
        try:
            ...
            if ignored_register_map_csv_columns:
                _log.warning("%s: Ignored register map csv columns: %s", name,
                             ','.join(ignored_register_map_csv_columns))
        except IndexError:
        ...
        modbus_client_class = Map(name=name,
                                  addressing=addressing,
                                  endian=endian,
                                  registry_config_lst=selected_registry_config_lst).get_class()
        ...
        if port:
            self.modbus_client.set_transport_tcp(hostname=device_address, port=port)
        else:
            self.modbus_client.set_transport_rtu(device=device_address,
                                                 baudrate=baudrate,
                                                 bytesize=bytesize,
                                                 parity=parity,
                                                 stopbits=stopbits,
        ...
        for reg_dict in selected_registry_config_lst:
            register = ModbusTKRegister(
                reg_dict.get('volttron point name'), reg_dict.get('default value', None),
